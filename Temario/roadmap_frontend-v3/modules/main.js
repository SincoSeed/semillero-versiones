...
myApp.require(['services', 'menus/main'], (servicios, menus) => {
    ...
    if (localStorage.getItem('basededatos') === '1') {
        ...
        fetch("http://golden/Semillero/Temario/V3/roadmap_frontend-v3/summaries.json").then(blob => blob.json()).then(temas => {
            fetch("http://golden/Semillero/Temario/V3/roadmap_frontend-v3/menus.json").then(blob => blob.json()).then(data => {
                let menusTotales = 0,
                    menusRegistrados = 0;
