...
var palabrasLocal = JSON.parse(localStorage.getItem("varPalabras"));
var iniciado = false;
var habilitado = true;
var palabraMostrar = '';
var cronotemp;
...
function tempo(sec) {
    cronotemp = setInterval(() => {
        (sec < 0) ? clearInterval(cronotemp) : countDown(sec);
        sec--;
    }, 1000);
}
...
function mostrarInicio() {
    ...
    document.querySelector('.instrucciones').classList.remove('visible')
}
...
function pasar(callBack) {
    if (!palabrasIncorrectas.some(p => p === palabraMostrar) && puntaje.correcto + puntaje.incorrecto < 10) {
        puntaje.incorrecto++;
        clearInterval(temporizador);
        clearInterval(cronotemp);
        palabrasIncorrectas.push(palabraMostrar);
        console.log(palabrasIncorrectas);
        reproducirAudio('wrong2.mp3');
        setTimeout(() => {
            iniciar();
            callBack();
...
function correcto(callBack) {
    if (!palabrasCorrectas.some(p => p === palabraMostrar) && puntaje.correcto + puntaje.incorrecto < 10) {
        puntaje.correcto++;
        clearInterval(temporizador);
        clearInterval(cronotemp);
        palabrasCorrectas.push(palabraMostrar);
...
function mostrarPalabra() {
    ...
    if (palabrasMostradas == null) {
        ...
        if (validarPalabra(palabraMostrar)) {
           console.log(`esta palabra ${palabraMostrar} ya salió`);
            if(palabrasLocal.length = palabrasMostradas.length){
                localStorage.removeItem('motradas');
            }
        } else {
            divPalabra.innerHTML = palabra;
            palabrasMostradas.push(palabra);
...
function mostrarResultado() {
    ...
    //     localStorage.removeItem('mostradas');
    // }
}
...
window.addEventListener('keydown', (e) => {
    if (!iniciado) {
        if (e.ctrlKey && (e.keyCode === 97 || e.keyCode === 65)) {
            ...
            e.preventDefault();
            if (puntaje.cantidad == 0) {
                soundstart();
            }
        }
    }
    
    else if (iniciado && habilitado && palabraMostrar !== '') {
        if (e.keyCode === 32) {            
            n = n ||0;
            n++;
            console.log(n);
            clickCorrectoyPasar(pasar)(e);
        }
        else if (e.keyCode === 13) {
            clickCorrectoyPasar(correcto)(e);
        }
    }
})
var n=0;
